...
def text_file_to_speech(source_path, output_path, tts_engine=None, *, alignment=None, speaker_wav=None, voice_map=None):
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, seg_speaker: str | None = None) -> tuple[int, bytes | None]:
            wav_path = str(pathlib.Path(synth_dir) / f"seg_{idx}.wav")
            # Per-segment voice: voice_map > speaker_wav > default
            seg_voice = None
            if voice_map and seg_speaker:
                seg_voice = voice_map.get(seg_speaker)
            elif speaker_wav:
                seg_voice = speaker_wav
            if seg_voice:
                engine.tts_to_file(text=text, file_path=wav_path, speaker_wav=seg_voice)
                raw = pathlib.Path(wav_path).read_bytes() if pathlib.Path(wav_path).exists() else None
                return idx, raw
